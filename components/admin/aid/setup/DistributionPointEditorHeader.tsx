...
interface DistributionPointEditorHeaderProps {
  onCancel: () => void
  onSave: () => void
  saving?: boolean
  isEdit?: boolean
...
export default function DistributionPointEditorHeader({
  onCancel,
  onSave,
  saving = false,
  isEdit = false,
}: DistributionPointEditorHeaderProps) {
    ...
    <header className="mb-6" dir="rtl">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 flex-1 text-right">
          <h1 style={ADMIN_PAGE_TITLE_STYLE}>إدارة المساعدات الإنسانية</h1>
          <p style={{ ...ADMIN_PAGE_SUBTITLE_STYLE, marginTop: '8px' }}>
            {isEdit ? 'تعديل نقطة التوزيع' : 'إضافة أو تعديل نقطة التوزيع'}
          </p>
        </div>
