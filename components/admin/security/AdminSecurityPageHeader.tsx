...
interface AdminSecurityPageHeaderProps {
  title: string
  subtitle: string
...
export default function AdminSecurityPageHeader({
  title,
  subtitle,
  action,
  mobileTabs,
}: AdminSecurityPageHeaderProps) {
  ...
  return (
    <header className="mb-4 min-w-0 sm:mb-6">
      ...
      >
        ...
        {action && (
          <div className="flex w-full min-w-0 shrink-0 flex-col gap-3 sm:gap-3 xl:w-auto xl:self-start">
            {action}
          </div>
        )}
      </div>
...
export function AdminSecurityPrimaryButton({
  children,
  onClick,
  className = '',
  disabled,
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-xs font-bold text-white transition-opacity hover:opacity-90 disabled:opacity-60 min-[480px]:w-auto sm:px-5 sm:py-3 sm:text-sm xl:w-auto ${className}`}
      style={{
        background: '#2196F3',
