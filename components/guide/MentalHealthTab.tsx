...
export default function MentalHealthTab() {
  ...
  const startBreathing = () => {
    ...
    setTimeout(() => { clearInterval(interval); setIsBreathing(false); setBreathPhase(0) }, 48000)
  }
    ...
    <div className="flex flex-col gap-6" style={{ direction: 'rtl' }}>
      ...
      <div className="bg-gradient-to-l from-[#1565C0] to-[#2196F3] rounded-[28px] p-6 sm:p-8 flex flex-col md:flex-row items-center gap-6 shadow-lg">
        <div className="flex flex-col items-center gap-2 flex-shrink-0">
          <div className={`w-24 h-24 sm:w-28 sm:h-28 rounded-full border-4 border-white/50 bg-white/20 flex items-center justify-center transition-all duration-[4000ms] ${
            isBreathing && breathPhase === 0 ? 'scale-125 bg-white/30' : isBreathing && breathPhase === 2 ? 'scale-90 bg-white/10' : 'scale-100'
          }`}>
            <span className="text-white font-black text-[18px]">{isBreathing ? PHASES[breathPhase] : 'ابدأ'}</span>
          </div>
          {isBreathing && (
            <div className="flex gap-2 mt-1">
              {[0,1,2].map(p => <span key={p} className={`w-2 h-2 rounded-full ${breathPhase === p ? 'bg-white' : 'bg-white/40'}`} />)}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-[24px] border border-slate-100 p-5 sm:p-6 shadow-sm flex flex-col gap-3">
          ...
          {MENTAL_TIPS.map((tip, i) => (
            <div key={i} className="border-b border-slate-50 last:border-0 pb-3 last:pb-0">
              ...
              {openTip === i && (
                <p className="text-slate-500 text-[12px] sm:text-[13px] font-bold leading-relaxed mt-2 pr-1">{tip.desc}</p>
              )}
            </div>
          ))}
        </div>
