...
export default function ClinicMapView({ clinic, onBack }: ClinicMapViewProps) {
  return (
    <div className="relative w-full h-full overflow-hidden" style={{ direction: 'rtl', fontFamily: "'Cairo', sans-serif" }}>
      ...
      <div className="absolute inset-0 bg-[#e5e7eb]">
        ...
        <div className="absolute top-[45%] right-[35%] flex items-center gap-2">
          ...
          <span className="text-amber-500 font-bold drop-shadow-md text-[14px] sm:text-[16px]">{clinic?.name || 'مستوصف النصر'}</span>
        </div>
      </div>
      ...
      <div className="absolute bottom-4 sm:bottom-6 left-4 right-4 sm:left-8 sm:right-auto w-auto sm:w-[480px] xl:w-[580px] bg-white rounded-[20px] sm:rounded-[24px] shadow-[0_10px_40px_rgba(0,0,0,0.2)] p-5 sm:p-6 xl:p-8 z-50 border border-slate-100">
        ...
        <h1 className="text-lg sm:text-2xl md:text-3xl font-black text-slate-800 mb-1 sm:mb-2 truncate">
          {clinic?.name || 'السويدي التابع للانوروا'}
        </h1>
        ...
        <div className="flex items-center gap-2 text-slate-500 font-bold text-[13px] sm:text-[15px] xl:text-[17px]">
          ...
          <span className="truncate">{clinic?.address || 'شارع النصر-غزة'}</span>
        </div>
      </div>
    </div>
  )
