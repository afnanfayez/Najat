    ...
    <Card className="p-5 sm:p-6 rounded-[24px] border border-slate-100 shadow-sm bg-white flex flex-col">
      ...
      <div className="flex flex-col gap-3 text-right">
        <div className="flex items-start gap-3 text-slate-700">
          ...
          <span className="font-black text-[13px] sm:text-sm leading-relaxed">{facility?.address || 'غزة-الرمال-مقابل مرطبات كاظم-شارع خالد بن الوليد'}</span>
        </div>
        <div className="flex items-center gap-3 text-slate-700">
          <Phone size={18} className="text-[#F2A122] flex-shrink-0 sm:w-[20px] sm:h-[20px]" />
          <span className="font-black text-[13px] sm:text-sm">{facility?.phone || '0592201453'}</span>
        </div>
      </div>
    </Card>
  )
